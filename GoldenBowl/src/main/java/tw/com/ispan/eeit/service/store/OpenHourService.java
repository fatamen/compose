...
public class OpenHourService {
	...
	public Optional<OpenHourDTO> getStoreTodayOpenHour(Integer storeId) {
		...
		Optional<OpenHourBean> todayOpenHours = openHourRepo.findByStoreAndDay(store, dayName);

		if (todayOpenHours.isEmpty()) {
			return Optional.empty(); // 今天沒有設定營業時間
		}

		System.out.println(OpenHourDTO.fromEntity(todayOpenHours.get()).getId());
		// 假設一天只有一組營業時間，取第一個。
	...
	public OpenHourBean updateOpenHour(OpenHourDTO hourDto) {
		...
		if (hourDto.getId() != null) {
			...
			// throw new IllegalArgumentException("OpenHour ID must be provided for update
			// operation.");
		}
	}
